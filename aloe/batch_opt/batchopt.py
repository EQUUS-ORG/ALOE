...
class FIRE(nn.Module):
    ...
    def __init__(self, shape: Tuple[int], device: str | torch.device = "cpu"):
...
def n_steps(
    ...
    for istep in tqdm(range(1, (n + 1), 1)):
        ...
        # Only build a grad graph for models that NEED autograd to get forces
        needs_grad = net.name in {
            "ANI2xt",
            "ANI2x",
            "AIMNET-lite",
        }  # AIMNET return forces directly
        coord.requires_grad_(needs_grad)

        # Forward
        e, f = net.forward_batched(coord, numbers, charges)

        # Clean up only if we enabled grads
        if needs_grad:
...
class optimizing(object):
    def __init__(self, in_f, out_f, name, device, config, ignore_rdkit_errors):
        self.in_f = in_f
        self.out_f = out_f
        self.name = name
        self.device = device
        self.config = config
        self.ignore_rdkit_errors = ignore_rdkit_errors
    ...
    def run(self):
        ...
        mols_raw = list(
            Chem.SDMolSupplier(self.in_f, removeHs=False)
        )  # may contain None
        if not self.ignore_rdkit_errors:
            mols = [mol for mol in mols_raw if mol is not None]
            if len(mols) < len(mols_raw):
                print(
                    f"Warning: {len(mols_raw) - len(mols)} molecules were skipped due to RDKit read errors",
                    flush=True,
                )
        else:
            mols = mols_raw
