...
class aloe:
    def __init__(self, input_file, output_dir=None, output_file = None, intermediate_dir=None, remove_intermediates=False, **kwargs):
        r"""
        Arguments:
        ...
        output_file: str, path to the output file, default None. Will override output_dir if provided.
        remove_intermediates: bool, whether to remove intermediate files, default False.
        ...
        memory: int, RAM allocation for ALOE in GB, default None.
        batchsize_atoms: int, Number of atoms per batch for geometry calculations, default 2048.
        ignore_rdkit_errors: bool, whether to ignore RDKit errors, default False.
        """
        self.input_file = os.path.abspath(input_file)
        self.output_dir = output_dir
        self.output_file = output_file
        basename = os.path.basename(self.input_file).split(".")[0].strip()
        self.intermediate_dir = (
            os.path.abspath(intermediate_dir)
            if intermediate_dir
            else os.path.join(
                os.path.dirname(self.input_file), f"{basename}_intermediates"
            )
        )
        ...
        self.hardware_settings = {
            ...
            "cpu_workers": kwargs.get("cpu_workers", 4),
        }
        self.ignore_rdkit_errors = kwargs.get("ignore_rdkit_errors", False)
    ...
    def prepwork(self):
        r"""
        Prepares the aloe pipeline for execution.
        This function can be used to ensure that all necessary parameters are set before running the pipeline.
        Returns:
            None
        """
        ...
        (
            ...
            self.hardware_settings["batchsize_atoms"],
        ) = update_hardware_settings(self.hardware_settings)
        ...
        chunks, chosen_intermediate_dir = save_chunks(
            self.input_file,
            self.intermediate_dir,
            configured_memory_gb,
            self.hardware_settings["do_batch"],
            batch_info,
        )

        self.intermediate_dir = chosen_intermediate_dir
        ...
        opt = self.user_parameters["OptConfig"]
        if "convergence_threshold" in opt and "opttol" not in opt:
            opt["opttol"] = opt["convergence_threshold"]
    ...
    def run(self):
        ...
        output_file = combine_files(
            output_files, self.input_file, self.output_dir, output_suffix, self.output_file
        )
...
def run_pipeline(input_file, pipeline, gpu_index):
    """
    Executes a sequence of functions (pipeline) on the input file using the specified GPU index.
    Args:
        input_file: str, path to the input file.
        pipeline: list of functions to execute sequentially.
        gpu_index: int, GPU index to use for processing.
    Returns:
        str, path to the final output file after processing through the pipeline.
    """
...
def gpu_worker(gpu_id, input_file, pipeline, result_queue):
    """
    Corresponds to a single GPU/CPU worker process.
    Args:
        gpu_id: int, GPU index to use for processing.
        input_file: str, path to the input file.
        pipeline: list of functions to execute sequentially.
        result_queue: multiprocessing.Queue, queue to store the output file path.
    Returns:
        None, puts the output file path into the result_queue.
    """
    if not gpu_id == -1:
...
def gpu_job_manager(jobs, available_gpus):
    """
    Distributes jobs across available GPUs and manages their execution.
    Args:
        jobs: list of tuples, each containing (input_file, pipeline).
        available_gpus: list of int, indices of available GPUs.
    Returns:
    ...
    while job_idx < len(jobs) or running > 0:
        ...
        if job_idx < len(jobs) and not gpu_pool.empty():
            gpu_id = gpu_pool.get()
            input_file, pipeline = jobs[job_idx]
            p = multiprocessing.Process(
                target=gpu_worker,
                args=(gpu_id, input_file, pipeline, result_queue),
            )
            p.start()
            job_idx += 1
            running += 1
...
def run_auto3D_pipeline(
    ...
    r"""
    Prepares the Auto3D pipeline on the given chunks of data.
    Args:
        ...
        gpu_indicies: list of int, GPU index or indices to use for processing.
    Returns:
        list of str, paths to output files.
    """
    ...
    for func in selected_functions:
        kwargs = dict(user_parameters[func])
        # Inject hardware_settings for the CPU-parallel steps
        if func in ("ConformerConfig", "RankConfig"):
            kwargs["hardware_settings"] = hardware_settings
        pipeline.append(functools.partial(FUNCTIONS[func], **kwargs))
