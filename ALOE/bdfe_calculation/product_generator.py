...
def generate_products(input_file, output_file):
    ...
    for key in SUBSTRUCTURES:
        ...
        match oxidation_state:
            case "oxidized":
                reduction_reactions[key] = AllChem.ReactionFromSmarts(f'{SUBSTRUCTURES[key]}>>{SUBSTRUCTURES[new_key]}')
            case "reduced":
                oxidization_reactions[key] = AllChem.ReactionFromSmarts(f'{SUBSTRUCTURES[key]}>>{SUBSTRUCTURES[new_key]}')
