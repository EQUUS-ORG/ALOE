...
class rd_enumerate_EZ_isomer(object):
    def __init__(
        self,
        csv: str,
        enumerated_csv: str,
        ...
        self.csv = csv
        self.enumerated_csv = enumerated_csv
        self.pattern = Chem.MolFromSmiles("N=C1C=CC=CC1=N")

    def EZ_helper(self, mol, bond_idx):
        """
    ...
    def enumerate_EZ_diimine(self, smi):
        """
        (E, E), (E, Z), (Z,E), (Z, Z)
        """

        mol = Chem.MolFromSmiles(smi)
        match = mol.GetSubstructMatches(self.pattern)[0]
        ...
        mol_copy = self.EZ_helper(mol_copy, [bond1_id, bond2_id])
        list_of_smiles.append(("EE", Chem.MolToSmiles(mol_copy)))
        ...
        mol_copy = self.EZ_helper(mol_copy, [bond1_id, bond2_id])
        list_of_smiles.append(("EZ", Chem.MolToSmiles(mol_copy)))
        ...
        mol_copy.GetBondWithIdx(bond2_id).SetStereo(Chem.rdchem.BondStereo.STEREOE)
        mol_copy = self.EZ_helper(mol_copy, [bond1_id, bond2_id])
        list_of_smiles.append(("ZE", Chem.MolToSmiles(mol_copy)))
        ...
        mol_copy.GetBondWithIdx(bond2_id).SetStereo(Chem.rdchem.BondStereo.STEREOZ)
        mol_copy = self.EZ_helper(mol_copy, [bond1_id, bond2_id])
        list_of_smiles.append(("ZZ", Chem.MolToSmiles(mol_copy)))
    ...
    def run(self):
        ...
        with open(self.enumerated_csv, "w", newline="") as csvfile:
            ...
            writer.writerow(["Name", "SMILES"])
            for key, val in data.items():
                isomers = self.enumerate_EZ_diimine(val)

                for code, smi in isomers:
                    writer.writerow([f"{key}-{code}", smi])
